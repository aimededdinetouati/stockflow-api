...
public class SaleOrderResource {
    ...
    /**
     * {@code GET  /sale-orders/search} : Search sale orders with advanced criteria.
     *
     * @param pageable the pagination information.
     ...
     */
    @GetMapping("/search")
    public ResponseEntity<List<SaleOrderDTO>> searchSaleOrders(
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        ...
        LOG.debug("REST request to search SaleOrders with query: {}", query);

        // Build specification based on parameters
        Specification<com.adeem.stockflow.domain.SaleOrder> spec = Specification.where(null);

        if (query != null && !query.trim().isEmpty()) {
            spec = spec.and(SaleOrderSpecification.withReferenceContaining(query));
        }

        if (status != null && !status.trim().isEmpty()) {
            try {
                spec = spec.and(
                    SaleOrderSpecification.withStatus(com.adeem.stockflow.domain.enumeration.OrderStatus.valueOf(status.toUpperCase()))
                );
        ...
        if (orderType != null && !orderType.trim().isEmpty()) {
            try {
                spec = spec.and(
                    SaleOrderSpecification.withOrderType(com.adeem.stockflow.domain.enumeration.OrderType.valueOf(orderType.toUpperCase()))
                );
            } catch (IllegalArgumentException e) {
                throw new BadRequestAlertException("Invalid order type value", ENTITY_NAME, "invalidOrderType");
            }
        }

        if (customerId != null) {
            spec = spec.and(SaleOrderSpecification.withCustomerId(customerId));
        }

        if (fromDate != null && toDate != null) {
            try {
                java.time.ZonedDateTime from = java.time.ZonedDateTime.parse(fromDate);
                java.time.ZonedDateTime to = java.time.ZonedDateTime.parse(toDate);
                spec = spec.and(SaleOrderSpecification.withDateRange(from, to));
            } catch (java.time.format.DateTimeParseException e) {
                throw new BadRequestAlertException("Invalid date format", ENTITY_NAME, "invalidDateFormat");
            }
        }

        Page<SaleOrderDTO> page = saleOrderService.findAllWithCriteria(pageable, spec);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}

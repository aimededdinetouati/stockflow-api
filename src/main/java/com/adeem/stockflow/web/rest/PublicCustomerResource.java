...
/**
 * REST controller for customer account validation and lookup.
 */
@RestController
@RequestMapping("/api/public/customer-lookup")
class PublicCustomerLookupResource {

    private static final Logger LOG = LoggerFactory.getLogger(PublicCustomerLookupResource.class);

    private final PublicCustomerService publicCustomerService;

    public PublicCustomerLookupResource(PublicCustomerService publicCustomerService) {
        this.publicCustomerService = publicCustomerService;
    }

    /**
     * {@code GET  /public/customer-lookup/by-phone} : Find customer by phone number.
     *
     * @param phone the phone number.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the customer, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/by-phone")
    public ResponseEntity<CustomerDTO> findCustomerByPhone(@RequestParam("phone") String phone) {
        LOG.debug("REST request to find customer by phone : {}", phone);

        CustomerDTO customer = publicCustomerService.findCustomerByPhone(phone);
        if (customer != null) {
    ...
    /**
     * {@code GET  /public/customer-lookup/check-availability} : Check email and phone availability.
     *
     * @param email the email to check.
     * @param phone the phone to check.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the availability results.
     */
    ...
    public ResponseEntity<Map<String, Boolean>> checkAvailability(
        ...
        LOG.debug("REST request to check availability - email: {}, phone: {}", email, phone);

        Map<String, Boolean> availability = new java.util.HashMap<>();

        if (email != null) {
            availability.put("emailAvailable", publicCustomerService.isEmailAvailable(email));
        }

        if (phone != null) {
            availability.put("phoneAvailable", publicCustomerService.isPhoneAvailable(phone));
        }
